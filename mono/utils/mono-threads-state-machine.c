...
retry_state_change:
	...
	case STATE_STARTING:
		...
		break;
	default:
		mono_thread_state_assertion_fail ("Cannot transition current thread from %s with ATTACH", state_name (cur_state));
	}
}
...
*/
	default:
		mono_thread_state_assertion_fail ("Cannot transition current thread %p from %s with DETACH", info, state_name (cur_state));
	}
}
...
*/
	default:
		mono_thread_state_assertion_fail ("Cannot transition thread %p from %s with SUSPEND_REQUEST", mono_thread_info_get_tid (info), state_name (cur_state));
	}
}
...
*/
	default:
		mono_thread_state_assertion_fail ("Cannot transition thread %p from %s with ASYNC_SUSPEND_REQUESTED", mono_thread_info_get_tid (info), state_name (cur_state));
	}
...
*/
	default:
		mono_thread_state_assertion_fail ("Cannot transition thread %p from %s with STATE_POLL", mono_thread_info_get_tid (info), state_name (cur_state));
	}
}
...
*/
	default:
		mono_thread_state_assertion_fail ("Cannot transition thread %p from %s with REQUEST_RESUME", mono_thread_info_get_tid (info), state_name (cur_state));
	}
}
...
*/
	default:
		mono_thread_state_assertion_fail ("Cannot transition thread %p from %s with FINISH_ASYNC_SUSPEND", mono_thread_info_get_tid (info), state_name (cur_state));
	}
}
...
*/
	default:
		mono_thread_state_assertion_fail ("Cannot transition thread %p from %s with COMPENSATE_FINISH_ASYNC_SUSPEND", mono_thread_info_get_tid (info), state_name (cur_state));
...
*/
	default:
		mono_thread_state_assertion_fail ("Cannot transition thread %p from %s with DO_BLOCKING", mono_thread_info_get_tid (info), state_name (cur_state));
	}
}
...
*/
	default:
		mono_thread_state_assertion_fail ("Cannot transition thread %p from %s with DONE_BLOCKING", mono_thread_info_get_tid (info), state_name (cur_state));
	}
}
