...
static int reuse_queue_size;
static int reuse_queue_peak_size;
...
reuse_sample_hit (gpointer p)
{
	...
	mono_lock_free_queue_node_free (&sample->node);
	mono_lock_free_queue_enqueue (&sample->prof->sample_reuse_queue, &sample->node);
	if (InterlockedIncrement (&reuse_queue_size) > InterlockedRead (&reuse_queue_peak_size))
		InterlockedIncrement (&reuse_queue_peak_size);
}
...
mono_sample_hit (MonoProfiler *profiler, unsigned char *ip, void *context)
{
	...
	if (sample && (hazardous = is_pointer_hazardous (sample))) {
...
log_shutdown (MonoProfiler *prof)
{
	...
	printf ("reuse queue peak length = %d\n", reuse_queue_peak_size);
	printf ("size of each entry = %d bytes\n", (int) SAMPLE_SLOT_SIZE (num_frames));
